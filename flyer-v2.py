...
def main():
    ...
    nha_size = 46
    nha_x = M + 26
    nha_baseline = band_y + band_h - 40
    left_text(c, "NHA", nha_x, nha_baseline, nha_font, nha_size, ORANGE)
    nha_w = c.stringWidth("NHA", nha_font, nha_size)

    # Contact info just under the NHA logo
    contact_x = nha_x
    left_text(c, "newheightsaquatics@gmail.com",
              contact_x, band_y + band_h - 65,
              "Helvetica", 9, WHITE)
    left_text(c, "(626) 277-8767",
              contact_x, band_y + band_h - 82,
              "Helvetica", 9, WHITE)

    # Brand name + tagline shifted down so they sit near the vertical center of the band
    label_x = nha_x + nha_w + 18  # always start labels clear of NHA
    label_top_y = band_y + band_h - 55
    label_sub_y = band_y + band_h - 75
    ...
    # Payment methods, centered under the BOOK NOW! pill
    pay_center_x = pill_x + pill_w / 2
    pay_top_y = pill_y - 10
    center_text(c, "WE ACCEPT", pay_center_x, pay_top_y,
                "Helvetica-Bold", 8, ORANGE)
    center_text(c, "Cash", pay_center_x, pay_top_y - 14,
                "Helvetica-Bold", 9, WHITE)
    center_text(c, "Venmo:  @NHAswim", pay_center_x, pay_top_y - 26,
                "Helvetica-Bold", 9, WHITE)
    center_text(c, "Zelle:  newheightsaquatics", pay_center_x, pay_top_y - 38,
                "Helvetica-Bold", 9, WHITE)
    center_text(c, "Apple Pay:  (626) 277-8767", pay_center_x, pay_top_y - 50,
                "Helvetica-Bold", 9, WHITE)
